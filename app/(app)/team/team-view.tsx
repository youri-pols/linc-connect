...
export function TeamView({
  ...
  const [selectedDiscipline, setSelectedDiscipline] =
    useState<TeamDiscipline | null>(null);
  ...
  const filteredMembers = useMemo(() => {
    return members.filter((m) => {
      ...
      if (selectedExpertise && !m.expertises.includes(selectedExpertise))
        return false;
      return true;
    });
  ...
  const activeFilterCount =
    (selectedDiscipline ? 1 : 0) + (selectedExpertise ? 1 : 0);
        ...
        <div className="flex flex-col">
          {sortedDisciplines.map((d) => (
            <FilterItem
              ...
              onClick={() =>
                setSelectedDiscipline(selectedDiscipline === d.id ? null : d.id)
              }
            />
          ))}
        </div>
        ...
        <div className="flex flex-col">
          {sortedExpertises.map((e) => (
            <FilterItem
              ...
              label={e.label}
              count={e.count}
              active={selectedExpertise === e.label}
              onClick={() =>
                setSelectedExpertise(
                  selectedExpertise === e.label ? null : e.label,
                )
              }
            />
          ))}
    ...
    <div className="flex h-full">
      <div className="flex-1 min-w-0 overflow-y-auto flex flex-col gap-6 p-4 lg:p-8">
        ...
        <div className="flex items-baseline justify-between gap-3 flex-wrap">
          <div className="flex items-baseline gap-3">
            ...
            <p className="text-body text-sm text-black/60">
              {filteredMembers.length} medewerkers
            </p>
          </div>
        ...
        {filteredMembers.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-stretch">
            {filteredMembers.map((member) => (
              <TeamCard
                ...
                fallbackPhotoUrl={currentUserPhotoUrl}
                disciplineLabel={
                  disciplineLabels.get(member.discipline) ?? member.discipline
                }
              />
            ))}
          </div>
          ...
          <p className="text-body text-sm text-black/60">
            Geen medewerkers gevonden voor deze combinatie van filters.
          </p>
        )}
      </div>
