 ...
 *
 *   avatar + name + (optional "JU" badge)    discipline label
 ...
 * button instead of the default "Profiel" link. The edit route
 * isn't wired yet — clicking it routes to the same detail page.
 */
export function TeamCard({ member, isCurrentUser, fallbackPhotoUrl, disciplineLabel }: TeamCardProps) {
  ...
  return (
    <div className="flex flex-col gap-6 h-full bg-white border border-black/10 rounded-lg shadow-card p-5">
      ...
      {/* Footer row: icon buttons + primary CTA. `mt-auto` pins it
          to the card's bottom so cards stay aligned in the grid even
          when one has more expertise tags (and a taller top block). */}
      <div className="mt-auto flex items-center justify-between gap-3 pt-6 border-t border-black/10">
        <div className="flex items-center gap-1.5">
          ...
          <IconButton href="https://calendar.google.com/calendar/u/0/r/eventedit" external icon="calendar_add_on" label={`Plan afspraak met ${member.name}`} disabled={isCurrentUser} />
        </div>
...
function IconButton({ href, external, icon, label, disabled }: IconButtonProps) {
  ...
  if (disabled || !href) {
    return (
      <button type="button" aria-label={label} aria-disabled={disabled || undefined} tabIndex={disabled ? -1 : undefined} className={className}>
